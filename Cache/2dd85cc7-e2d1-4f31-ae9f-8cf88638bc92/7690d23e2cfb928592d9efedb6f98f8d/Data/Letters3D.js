...
//@input float lerpSpeedMovement = 0.01
//@input float lerpSpeedRotation = 0.01
...
function Update() {
  ...
  var newPos = vec3.lerp(currentPos, parentPos, script.lerpSpeedMovement);
  script.letter3DGroup.getTransform().setWorldPosition(newPos);

  // UPDATE THE ROTATION
  parentRot = parentTr.getWorldRotation();
  var currentRot = letter3DGroupTr.getWorldRotation();
  var newRot = quat.slerp(currentRot, parentRot, script.lerpSpeedRotation);
